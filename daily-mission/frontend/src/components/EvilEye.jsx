  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (program) {
        ...
        program.uniforms.uResolution.value = [w, h, w / Math.max(h, 1)];
      }
    };
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: [gl.drawingBufferWidth, gl.drawingBufferHeight, gl.drawingBufferWidth / Math.max(gl.drawingBufferHeight, 1)] },
    ...
    return () => {
      ...
      // Note: ogl Mesh objects do not have a .remove() method.
      // Geometry and Program manage the actual WebGL resources.
      
      if (renderer && typeof renderer.destroy === 'function') {
        renderer.destroy();
      }
    };
  }, [eyeColor, intensity, pupilSize, irisWidth, glowIntensity, scale, noiseScale, pupilFollow, flameSpeed, backgroundColor, dpr, paused]);
